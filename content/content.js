...
>>>>>>> Stashed changes
   ...
   function readAmazonContext() {
      // Product title
      const name =
         qsText("#productTitle") || qsText("h1.product-title-word-break");
...
>>>>>>> Stashed changes
   ...
   function readWalmartContext() {
      try {
         ...
         console.warn("[TuffyReviews] scraping error:", err);
         return { retailer: "walmart", name: null, upc: null };
      }
   }
   ...
   function readEbayContext() {
      const name =
         qsText(".x-item-title__mainTitle .ux-textspans--BOLD") ||
         qsText("h1.x-item-title");
      // eBay item ID from URL
      const itemMatch = window.location.pathname.match(/\/itm\/(\d+)/);
      return {
      ...
      cards.forEach((card) => {
         ...
         try {
            ...
            reviews.push({
               ...
               rating: ratingMatch ? parseFloat(ratingMatch[1]) : null,
               date: dateRaw
                  ? dateRaw.replace(/Reviewed in .+ on /i, "").trim()
                  : null,
               title: qsText(
   ...
   function readAmazonCustomersSay() {
      ...
      const headingEl =
         Array.from(
            document.querySelectorAll("h1,h2,h3,h4,span,div,section"),
         ).find((el) => {
            const t = el.textContent ? el.textContent.trim() : "";
            return (
               t && customersRe.test(t) && t.toLowerCase() === "customers say"
            );
         }) ||
         Array.from(
            document.querySelectorAll("h1,h2,h3,h4,span,div,section"),
            ...
            return (
               t &&
               customersRe.test(t) &&
               t.toLowerCase().startsWith("customers say")
            );
         });
      ...
      const extracted =
         end >= 0 ? full.slice(afterHeading, end) : full.slice(afterHeading);

      let cleaned = extracted.replace(/^\s*[:\-–—]?\s*/, "").trim();

      // Deduplicate "AI Generated from the text of customer reviews" if repeated.
      const aiPhraseRe =
         /ai\s+generated\s+from\s+the\s+text\s+of\s+customer\s+reviews/i;
   ...
   function readAmazonPrice() {
      ...
      return {
         current: parsePrice(currentRaw),
         original: parsePrice(originalRaw),
         currency: "USD",
      };
   }
   ...
   function readWalmartReviews() {
      const data = getWalmartNextData();
      if (!data) return [];
      const rc = findInObject(
         data,
         (o) =>
            Array.isArray(o.reviews) &&
            o.reviews.length > 0 &&
            o.reviews[0].reviewText !== undefined,
         20,
      );
      if (!rc) return [];
      return rc.reviews.slice(0, 5).map((r) => ({
         ...
         rating:
            typeof r.rating === "number"
               ? r.rating
               : parseFloat(r.rating) || null,
         date: r.reviewSubmissionTime
            ? new Date(r.reviewSubmissionTime).toLocaleDateString("en-US", {
                 year: "numeric",
                 month: "long",
                 day: "numeric",
              })
   ...
   function readWalmartPrice() {
      const data = getWalmartNextData();
      if (!data) return { current: null, original: null, currency: "USD" };
      const priceObj = findInObject(
         data,
         (o) =>
            (typeof o.currentPrice === "number" ||
               typeof o.price === "number") &&
            o.currencyUnit !== undefined,
         20,
      );
      if (priceObj)
         return {
      ...
      return {
         current: parsePrice(qsText('[itemprop="price"]')),
         original: null,
         currency: "USD",
      };
   }
   ...
   function readEbayReviews() {
      ...
      feedbackItems.forEach((item) => {
         ...
         reviews.push({
            ...
            body: comment,
            verified: true,
            source: "eBay (Seller Feedback)",
         });
      });
      if (!reviews.length) {
         const scoreEl =
            qs(
               '.x-sellercard-atf__data-item-wrapper [data-testid="x-sellercard-atf__data-item"] a',
            ) || qs('[data-testid="ux-seller-section__item--seller"] a');
         const score = scoreEl ? scoreEl.textContent.trim() : null;
         if (score)
            reviews.push({
               reviewer: "Seller Rating",
               rating: null,
               date: null,
               title: "eBay Seller Feedback Score",
               body: `This seller has a feedback score of ${score}. Full feedback is on the seller's eBay page.`,
   ...
   function readEbayPrice() {
      const currentRaw =
         qsText(".x-price-primary > span") ||
         qsText(".x-price-primary") ||
         qsText('[data-testid="x-price-primary"] span');
      const originalRaw =
         qsText(".x-price-approx__price") ||
         qsText('[data-testid="x-price-approx"] span');
      return {
         current: parsePrice(currentRaw),
         original: parsePrice(originalRaw),
         currency: "USD",
   ...
   const REVIEW_READERS = {
      amazon: readAmazonReviews,
      walmart: readWalmartReviews,
      ebay: readEbayReviews,
   };
   const PRICE_READERS = {
      amazon: readAmazonPrice,
      walmart: readWalmartPrice,
      ebay: readEbayPrice,
   };
      ...
      chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
         if (message.type === "GET_REVIEWS") {
            try {
               ...
               console.warn("[TuffyReviews] GET_REVIEWS failed:", err);
               sendResponse({
                  retailer,
                  url: window.location.href,
                  reviews: [],
         ...
         if (message.type === "GET_PRICE") {
            sendResponse({
               retailer,
               url: window.location.href,
               ...PRICE_READERS[retailer](),
            });
            return true;
         }
         if (message.type === "GET_CONTEXT") {
            sendResponse({
               ...CONTEXT_READERS[retailer](),
               url: window.location.href,
            });
            return true;
         }
